...
struct SegmentTree {
    ...
    void upd(int s, int e, lazy_t v) {upd(s, e, v, 0, 0, n-1);}
    void upd(int s, int e, lazy_t v, int u, int l, int r) {
        ...
        int m = (l + r) / 2;
        upd(s, e, v, 2*u+1, l, m);
        upd(s, e, v, 2*u+2, m+1, r);
        st[u] = st[2*u+1] + st[2*u+2];
    }
    node_t query(int s, int e) {return query(s, e, 0, 0, n-1);}
    node_t query(int s, int e, int u, int l, int r) {
        push(u, l, r);
        if(e < l || r < s) return node_t();
        if(s <= l && r <= e) return st[u];
        int m = (l + r) / 2;
        return query(s, e, 2*u+1, l, m) + query(s, e, 2*u+2, m+1, r);
    }
    template <class F> //first index >= i that satisfies F (monotone)
    int first_index(int i, F f) { return first_index<F>(i, 0, 0, n-1, f); }
    template <class F>
    int first_index(int i, int u, int l, int r, F f) {
        ...
        if(l >= i) {
            if(!f(st[u])) return -1;
            while(l != r) {
                push(2*u+1, l, m); push(2*u+2, m+1, r);
                if(f(st[2*u+1])) u = 2*u+1, r = m;
                else u = 2*u+2, l = m+1;
                m = (l + r) / 2;
            }
            return l;
        }
        int left_attempt = first_index(i, 2*u+1, l, m, f);
        if(left_attempt != -1) return left_attempt;
        return first_index(i, 2*u+2, m+1, r, f);
    }
    template <class F> //last index <= i that satisfies F (monotone)
    int last_index(int i, F f) { return last_index<F>(i, 0, 0, n-1, f); }
    template <class F>
    int last_index(int i, int u, int l, int r, F f) {
        ...
        if(r <= i) {
            ...
            while(l != r) {
                push(2*u+1, l, m); push(2*u+2, m+1, r);
                if(f(st[2*u+2])) u = 2*u+2, l = m+1;
                else u = 2*u+1, r = m;
                m = (l + r) / 2;
            }
            return l;
        }
        int right_attempt = first_index(i, 2*u+2, m+1, r, f);
        if(right_attempt != -1) return right_attempt;
        return last_index(i, 2*u+1, l, m, f);
    }
};
